...
export default function AdminKycPage() {
  ...
  async function updateStatus(id: number, status: string) {
    try {
      await fetch("/api/admin/kyc", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, status }) });
      toast.success("स्थिति अद्यावधिक भयो।"); fetchData();
  ...
  return (
    <AdminLayout>
      <div className="space-y-5 max-w-6xl">
        <div>
          <h1 className="text-xl font-bold text-gray-900 flex items-center gap-2"><FileText className="w-5 h-5" /> KYC आवेदनहरू</h1>
        ...
        <DataTable loading={loading} data={data.items} emptyText="कुनै KYC आवेदन छैन।"
          columns={[
            { key: "trackingId", header: "ट्र्याकिङ", render: (row) => <span className="font-mono text-xs">{row.trackingId}</span> },
            ...
            { key: "actions", header: "कार्य", render: (row) => row.status === "pending" ? (
